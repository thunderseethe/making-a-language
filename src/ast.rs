...
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash)]
pub struct Operation(pub usize);
...
pub enum Ast<V> {
  ...
  // --- Effect Nodes ---
  // Handle an effect produced by body with handler
  Handle(
    Box<Self>, // handler
    Box<Self>, // body
  ),
...
impl<V> Ast<V> {
  ...
  pub fn handle(handler: Self, body: Self) -> Self {
    Self::Handle(Box::new(handler), Box::new(body))
  }
}
