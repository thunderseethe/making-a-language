...
use crate::ty::{ClosedRow, Data, Eff, Row, RowCombination, RowVar, Type, TypeVar};
...
impl TypeInference {
  ...
  pub(crate) fn substitute_row<R: RowStyle>(&mut self, row: Row) -> SubstOut<Row> {
    match row {
      Row::Open(var) => {
  ...
  pub(crate) fn substitute_ty(&mut self, ty: Type) -> SubstOut<Type> {
    match ty {
      ...
      Type::Prod(row) => self.substitute_row::<Data>(row).map(Type::Prod),
      Type::Sum(row) => self.substitute_row::<Data>(row).map(Type::Sum),
    }
  }
  ...
  pub(crate) fn substitute_ast(&mut self, ast: Ast<TypedVar>) -> SubstOut<Ast<TypedVar>> {
    match ast {
      ...
      Ast::Unit => SubstOut::new(Ast::Unit),
      ...
      Ast::Handle(handler, body) => 
          self.substitute_ast(*handler)
              .merge(self.substitute_ast(*body), Ast::handle),
      Ast::Operation(op_name) => SubstOut::new(Ast::Operation(op_name)),
    }
  }

  pub(crate) fn substitute_data_row_comb(&mut self, comb: RowCombination<Data>) -> SubstOut<Evidence> {
    self
      .substitute_row::<Data>(comb.left)
      .merge(self.substitute_row::<Data>(comb.right), |l, r| (l, r))
      .merge(self.substitute_row::<Data>(comb.goal), |(left, right), goal| {
        Evidence::DataEquation { left, right, goal }
      })
  }

  pub(crate) fn substitute_eff_row_comb(&mut self, comb: RowCombination<Eff>) -> SubstOut<Evidence> {
    self
      .substitute_row::<Eff>(comb.left)
      .merge(self.substitute_row::<Eff>(comb.right), |l, r| (l, r))
      .merge(self.substitute_row::<Eff>(comb.goal), |(left, right), goal| {
        Evidence::EffEquation { left, right, goal }
      })
  }
}
